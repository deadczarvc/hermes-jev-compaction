...
const { messages, systemTexts } = fromHermes(hermesMessages);
...
if (opts.dryRun) {
  ...
  const payload = JSON.stringify({ messages: toHermes(messages), stats }, null, 2);
...
try {
  ...
  const payload = JSON.stringify({ messages: toHermes(result.messages), stats: result.stats }, null, 2);
  if (opts.out) writeFileSync(opts.out, payload);
  else console.log(payload);
